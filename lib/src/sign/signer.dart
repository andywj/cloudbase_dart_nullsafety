...
class Signer {
  ...
  tc3sign(
    ...
    final formatedHeaders =
        _formatKeyAndValue(headers, enableValueToLowerCase: true);
    final headerKV = SortedKeyValue(formatedHeaders);
    final signedHeaders = headerKV.keys;
    final canonicalHeaders =
        headerKV.toString(kvSeparator: ':', joinSeparator: '\n') + '\n';
    ...
    if (enableHostCheck && headerKV.get(HOST_KEY) != urlInfo.host) {
      throw Exception(
          'host:${urlInfo.host} in url must be equals to host:${headerKV.get('host')} in headers');
    }
    ...
    final multipart =
        headerKV.get(CONTENT_TYPE_KEY)?.startsWith(MIME_MULTIPART_FORM_DATA) ??
            false;
    final formatedParams = method.toUpperCase() == 'GET'
        ? ''
        : _formatKeyAndValue(params, multipart: multipart);
    ...
    final authorization =
        '$algorithm Credential=${this.secretId}/$credentialScope,${withSignedParams ? signedParamsStr : ''} SignedHeaders=${signedHeaders.join(';')}, Signature=$signature';
  ...
  static sign({
    ...
    final singer =
        Signer(secretId: secretId, secretKey: secretKey, service: 'tcb');
    ...
    final signatureInfo = singer.tc3sign(
        ...
        params: params ?? {},
        timestamp: timestamp,
        options: {'withSignedParams': withSignedParams});
