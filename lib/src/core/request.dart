...
class CloudBaseRequest {
  ...
  Future<CloudBaseResponse> post(
    ...
    final Response response = await _dio.post(_url,
        data: data,
        queryParameters: {'env': _core.config?.env},
        options: Options(
            headers: _getHeader('POST', data, true),
            contentType: 'application/json',
            responseType: ResponseType.json,
            sendTimeout: _core.config?.timeout ?? _TCB_DEFAULT_TIMEOUT));
  ...
  _getHeader(String method, params, bool withSignedParams) {
    final Map<String, dynamic> headers = {
      ...
      'Host': Uri.parse(_url).host,
    };
    final timestamp = (DateTime.now().millisecondsSinceEpoch / 1000).round();

    final signInfo = Signer.sign(
        secretId: _core.config?.secretId,
        secretKey: _core.config?.secretKey,
        method: method.toLowerCase(),
        url: '$_url?env=${_core.config?.env}',
        headers: headers,
        params: params,
        timestamp: timestamp,
