...
class Document {
  ...
  /// 指定显示或者不显示哪些字段
  late Map<String, dynamic> _projection;
  ...
  Future<CloudBaseResponse> _docRequest(
    ...
    params.addAll(
        {'collectionName': _coll, 'queryType': 'DOC', 'databaseMidTran': true});
  ...
  Future<DbUpdateResponse> set(dynamic data) async {
    if (_id == null) {
      return DbUpdateResponse(
          code: CloudBaseExceptionCode.INVALID_PARAM, message: 'docId不能为空');
    }
    ...
    Map<String, dynamic> query = {'_id': _id};
    Map<String, dynamic> params = {
      'query': query,
      'data': data,
      'multi': false,
    ...
    CloudBaseResponse res =
        await _docRequest('database.updateDocument', params);
    if (res.code != null) {
      return DbUpdateResponse(
          code: res.code, message: res.message, requestId: res.requestId);
    }
  ...
  Future<DbRemoveResponse> remove() async {
    ...
    CloudBaseResponse res =
        await _docRequest('database.deleteDocument', params);
    if (res.code != null) {
      return DbRemoveResponse(
          code: res.code, message: res.message, requestId: res.requestId);
    }
  ...
  Document field(Map<String, bool> projection) {
    ...
    return Document(
        core: _core, coll: _coll, docId: _id, projection: newProjection);
  }
}
